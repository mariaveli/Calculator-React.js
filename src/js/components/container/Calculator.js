...
class Calculator extends React.Component{
    ...
    changeView(display){
        ...
        if(result.length > 18){
            document.getElementById("display").style["text-align"] = "left";
        }else{
            document.getElementById("display").style["text-align"] = "right";
        }
        this.setState({
            disValue: result,
            disValue2: result2
    ...
    render() {
        return (
            <div id="calculus-pad">
                <h6>Calculatius Electronius</h6>
                <CalculatorScreen disValue={this.state.disValue} disValue2={this.state.disValue2}/>
                <div id="dis-but">
                    <div className = "displayButtons">
                        {buttons.map(obj => <CalculatorButton changeView={this.changeView.bind(this)} keyTrigger={obj.keyTrigger}  value={obj.value} keyCode={obj.keyCode} id={obj.id}/>)}
                    </div>
                </div>
            </div>
